  ...
  try {
    ...
    const projectsWithStats = await Promise.all(
      projects.map(async (project) => {
        const totalVisits = await Visit.countDocuments({ project: project._id });
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        operatingSystems: osStats.map(os => ({ name: os._id, count: os.count })),
        topCountries: countryStats.map(c => ({ name: c._id, count: c.count }))
      }
    });
  } catch (error) {
